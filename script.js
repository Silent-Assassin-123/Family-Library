...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Ind3cm92eHdiZnBremJtY2psc3NkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTAxNzY0NjIsImV4cCI6MjA2NTc1MjQ2Mn0.b2JkigbElvXnnntwve_7FWdhBSYfAgvoi6SVUahI6U';
...
async function updateUIForUser(user) {
    ...
    if (user) {
        ...
        if (profileError && profileError.code !== 'PGRST116') { // PGRST116 means no rows found (new user)
            ...
            // Removed: showModal('Error fetching user profile.');
            isAdmin = false;
        ...
        console.log("No current user, showing auth section (login page)."); // Debug log
        authSection.classList.remove('hidden'); // Show the login form
        ...
        currentUser = null;
        isAdmin = false;
        // Removed: showModal('Please sign in to access the library.'); // This previously showed the modal
    }
}
...
async function checkSessionAndRender() {
    console.log("checkSessionAndRender called.");
